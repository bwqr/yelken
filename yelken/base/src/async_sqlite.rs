use diesel::{
    ...
    query_builder::{CollectedQuery, MoveableBindCollector, QueryBuilder, QueryFragment, QueryId},
    row::IntoOwnedRow,
};
use diesel_async::{
    AsyncConnection, SimpleAsyncConnection, TransactionManager,
...
impl AsyncConnection for AsyncSqliteConnection {
            ...
            + 'query,
    {
        let rows = self.execute_with_prepared_query(source.as_query(), |conn, query| {
            ...
            let mut cache = <<<SqliteConnection as LoadConnection>::Row<'_, '_> as IntoOwnedRow<
                <SqliteConnection as Connection>::Backend,
...
impl AsyncSqliteConnection {
    fn execute_with_prepared_query<'a, MD, Q, R>(
        &'a mut self,
        query: Q,
        callback: impl FnOnce(&mut SqliteConnection, &CollectedQuery<MD>) -> QueryResult<R> + Send + 'a,
    ) -> QueryResult<R>
    where
